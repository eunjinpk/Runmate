...
object PlaceRepository {
    ...
    private const val PREFS_NAME = "runmate_favorites"

    val parks = mutableListOf(
        HanRiverPark(1, "여의도", "여의나루역 · 코스 4", 37.5285, 126.9335, R.drawable.map_yeouido, "강남"),
        HanRiverPark(2, "반포", "신반포역 · 코스 3", 37.4970, 126.9970, R.drawable.map_banpo, "강남"),
        HanRiverPark(3, "뚝섬", "뚝섬유원지역 · 코스 3", 37.5310, 127.0669, R.drawable.map_ttukseom, "강북"),
        HanRiverPark(4, "잠실", "잠실나루역 · 코스 4", 37.5196, 127.0827, R.drawable.map_jamsil, "강남"),
        HanRiverPark(5, "망원", "망원역 · 코스 2", 37.5556, 126.8983, R.drawable.map_mangwon, "강북"),
        HanRiverPark(6, "이촌", "이촌역 · 코스 3", 37.5183, 126.9722, R.drawable.map_ichon, "강북"),
        HanRiverPark(7, "잠원", "잠원역 · 코스 2", 37.5219, 127.0143, R.drawable.map_jamwon, "강남"),
        HanRiverPark(8, "광나루", "광나루역 · 코스 2", 37.5459, 127.1104, R.drawable.map_gwangnaru, "강북"),
        HanRiverPark(9, "난지", "월드컵경기장역 · 코스 3", 37.5697, 126.8791, R.drawable.map_nanji, "강북"),
        HanRiverPark(10, "양화", "당산역 · 코스 2", 37.5384, 126.9017, R.drawable.map_yanghwa, "강북"),
        HanRiverPark(11, "강서", "가양역 · 코스 2", 37.5658, 126.8309, R.drawable.map_gangseo, "강북")
    )

    fun loadFavorites(context: Context) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        parks.forEach { park ->
            park.isFavorite = prefs.getBoolean("fav_${park.id}", false)
        }
    }

    fun toggleFavorite(context: Context, parkId: Int) {
        val park = parks.find { it.id == parkId } ?: return
        park.isFavorite = !park.isFavorite
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        prefs.edit().putBoolean("fav_${parkId}", park.isFavorite).apply()
    }
    ...
    fun getFilteredParks(filter: String): List<HanRiverPark> {
        return when (filter) {
            "전체" -> parks
            "강북" -> parks.filter { it.region == "강북" }
            "강남" -> parks.filter { it.region == "강남" }
            "즐겨찾기" -> parks.filter { it.isFavorite }
            else -> parks
        }
    }
